...
def train(config, device, eval_only=False):
    ...
    print(config)
    print("")
    log_dir, ckpt_dir, video_dir, vis_dir, rollout_dir = TrainUtils.get_exp_dir(config)
    ...
    SIR_config = None
    if args.sir_config is not None:
        SIR_config = json.load(open(args.sir_config, 'r'))
        with open(os.path.join(log_dir, '..', 'sir_config.json'), 'w') as f:
            json.dump(SIR_config, f, indent=4)
    ...
    for epoch in range(0, config.train.num_epochs + 1): # epoch numbers start at 1        
        ...
        if config.experiment.rollout.enabled and (epoch > config.experiment.rollout.warmstart) and rollout_check:
            ...
            if args.sir_config is not None:
                ...
                SIR_predictor.eval()
                SIR_predictor.to(device)

                # exp_dir = rollout_dir.split("/rollout")[0]
                # with open(os.path.join(exp_dir, "SIR_config.json"), 'w') as f:
                #     json.dump(SIR_config, f, indent=4)
            else:
                SIR_predictor = None
                SIR_config = None
            ...
            SIR_sample_num = args.SIR_sample_num
            robot_centric = args.robot_centric
            ...
            if args.original:
                ...
                all_rollout_logs, video_paths = TrainUtils.rollout_with_stats_for_SIR(
                    policy=rollout_model,
                    envs=env_iterator(),
                    horizon=eval_env_horizon_list,
                    use_goals=config.use_goals,
            ...
            if args.eval_only:
                ...
                print(colored(f"eval_only mode. Finished rollout {config.experiment.rollout.n} times, break!", "green"))
                break
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="(optional) path to a config json that will be used to override the default settings for SIR predictor. \
            If omitted, default settings are used. This is the preferred way to run experiments.",
    )
    ...
    parser.add_argument(
        "--original",
        action='store_true',
        help="use original rollout without SIR or randomization"
    )

    parser.add_argument(
        "--randomize_base",
        action='store_true',
        help="randomize base position of the robot"
    )

    parser.add_argument(
        "--inference_mode",
        action='store_true',
        help="use inference mode for rollout"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="number of SIR samples to rollout"
    )

    parser.add_argument(
        "--robot_centric",
        action='store_true',
        help="use robot-centric origin for rollout"
    )
