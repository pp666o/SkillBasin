...
def collect_human_trajectory(
    ...
    ################################# START OF DEBUGGING ################################# by kx
    from robosuite.utils.camera_utils import get_camera_extrinsic_matrix
    from scipy.spatial.transform import Rotation
    arm_pos = env.sim.data.qpos[env.robots[0]._ref_arm_joint_pos_indexes]
    # print("arm_pos: ", arm_pos)
    arm_joint_pos_for_detect = [ 0.00748372, -1.28215526, -0.03359372, -1.71793475, -0.01200928,  1.37912621, 0.75347592]
    arm_joint_pos_for_manipulation = np.array(env.robots[0].init_qpos)
    def arm_fake_controller(target):
        maximum_step = 0.05
        threshold = 0.01
        while True:
            ...
            if np.all(np.abs(arm_joint_pos_error) < threshold):
                break
    print("arm move for detect")
    arm_fake_controller(arm_joint_pos_for_detect)
    ################################# END OF DEBUGGING #################################
    ...
    ################################# START OF DEBUGGING ################################# by kx
        camera_extrinsic = get_camera_extrinsic_matrix(env.sim, "robot0_eye_in_hand", apply_correction=False)
        ...
        pos, quat = convert_extrinsic_to_pos_and_quat(camera_extrinsic)
        # print("*"*50,"\ncamera pos: ", pos, "\ncamera quat: ", quat)
        # print("robot pos: ", env.sim.data.qpos[env.robots[0]._ref_base_joint_pos_indexes])
    ################################# END OF DEBUGGING #################################
